 ...
 * @endcode
 */
$settings['hash_salt'] = 'EUveZoyJNddC0ktm0pumJRmwEWIdwU0yiaa2NfCD4cQEbOmDAMsWv14MtTKiVs6Mq8i2lzPcfw';
...
// For Drush 9
$_SERVER['PWD'] = $app_root;

// Acquia DB
if (file_exists('/var/www/site-php')) {
  require '/var/www/site-php/gsc/gsc-settings.inc';
}
...
// Put here to override 'sync' from acquia cloud.
$settings['config_sync_directory'] = '../config/default';
$settings['config_vcs_directory'] = '../config/default';

// @TODO: May need to change once prod is there.
if (isset($_ENV['AH_SITE_ENVIRONMENT'])) {
  ...
  switch ($_ENV['AH_SITE_ENVIRONMENT']) {
    ...
    case 'dev':
      ...
      // Shield
      $config['shield.settings']['credentials']['shield']['user'] = 'webmaster';
      $config['shield.settings']['credentials']['shield']['pass'] = 'p!0n3er$';

      // Environment Indicator
      $config['environment_indicator.indicator']['bg_color'] = '#00990d';
    ...
    case 'test':
      ...
      // Environment Indicator
      $config['environment_indicator.indicator']['bg_color'] = '#2a5dff';
      $config['environment_indicator.indicator']['fg_color'] = '#FFFFBB';
 ...
 */
function acquia_search_override() {
...
/**
 * Sets whether the search should have read-only mode on (dev, test) or off (prod)
 */
function acquia_search_read_write() {
  $ah_env = @$GLOBALS['gardens_site_settings']['env'] ?: @$_ENV['AH_SITE_ENVIRONMENT'];
  $chosen_ro = false;
  if (!empty($ah_env) && isset($ah_env)) {
    if($ah_env == 'prod') {
      $chosen_ro = false;
    } else {
      $chosen_ro = true;
...
$config['acquia_search.settings']['override_search_core'] = acquia_search_override();
$config['acquia_search.settings']['read_only'] = acquia_search_read_write();
